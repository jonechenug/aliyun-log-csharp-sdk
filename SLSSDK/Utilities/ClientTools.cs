...
namespace Aliyun.Api.LOG.Utilities
{
    internal class LogClientTools
    {
        public static byte[] CompressToZlib(byte[] buffer)
        {
            ...
            {
                using (ZlibStream gz = new ZlibStream(compressStream, Ionic.Zlib.CompressionMode.Compress, Ionic.Zlib.CompressionLevel.Level6, true))
                {
        ...
        public static byte[] DecompressFromZlib(Stream stream, int rawSize)
        {
            using (stream)
            {
                using (ZlibStream dz = new ZlibStream(stream, Ionic.Zlib.CompressionMode.Decompress))
                {
        ...
        {
            return LZ4Codec.EncodeHC(buffer,0, buffer.Length);
        }
