# 简介
基于阿里原生SDK源码修改，将原有的dll依赖引用改为Nuget依赖，实测4.6.1通过。
```
// 简易用法
// 单条日志
new Aliyun.Api.LOG.Logger(config).PutLogs(topic,logModel);
// 多条日志
new Aliyun.Api.LOG.Logger(config).PutLogs(topic,logModelList);
```

# Aliyun LOG SDK FOR csharp
- [阿里云日志SDK源码](https://github.com/aliyun/aliyun-log-csharp-sdk?spm=a2c4g.11186623.2.3.a6SGfX)
